...
from ..utils.hangdang import normalize_hangdang
...
def _correlations(
    ...
    for ch in role.get("characters") or []:
        hd = normalize_hangdang(ch.get("hangdang_final"))
        if not hd or hd == "未知":
            continue
        peaks = [
            a
            ...
            out.append(
                {
                    ...
                    "evidence": f"{hd} 行当在 block {peaks[0]['block_index']} 附近情感强度较高",
                }
            )
            break
...
def _summary_insights(
    ...
    if stages:
        climax = next((s["block_range"] for s in stages if s["stage"] == "高潮"), stages[-1]["block_range"])
        insights.append(
            f"叙事划分为 {len(stages)} 个阶段，高潮位于 block {climax}。"
        )
